...
root.geometry("324x650")  # setting dimensions of window
...
# creating empty dictionary to store each cell of the input grid
cells = {}


# validate function to control what is being entered into the cells
def ValidateNumber(P):
    out = (P.isdigit() or P == "") and len(P) < 2
    return out


# registering the function to window
...
# To clear values in each cell of the grid
def clearValues():
...
def getValues():
    # declaring empty list to store the values for each cell for each row
    board = []
    errLabel.configure(text="")
    solvedLabel.configure(text="")
    for row in range(2, 11):
...
# creating update function to update the cells and to display the sudoku solution
def updateValues(s):
    sol = solver(s)
    if sol != "no":
